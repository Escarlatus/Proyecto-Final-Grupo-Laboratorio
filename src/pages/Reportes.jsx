...
import {
  BarChart2, ClipboardList, Download, Search, Filter, RefreshCw,
  FileText, CheckCircle2, Clock, AlertTriangle, XCircle, Stamp,
...
export default function Reportes() {
  ...
  useEffect(() => { fetchAll() }, [])
  useEffect(() => { if (activeTab === "auditoria" && auditLogs.length === 0) fetchAudit() }, [activeTab])
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Reportes y Auditoría</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                {Object.keys(stats.byType).length === 0 && <p className="text-sm text-slate-400 text-center py-4">Sin datos</p>}
              </div>
            </div>
              ...
              {Object.keys(stats.byMonth).length === 0 ? (
                ...
                <div className="flex items-end gap-2 h-32 overflow-x-auto">
                  {Object.entries(stats.byMonth).slice(-12).map(([month, count]) => {
                    ...
                    return (
                      <div key={month} className="flex flex-col items-center gap-1 shrink-0 flex-1 min-w-[40px]">
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{count}</span>
                        <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-t-md overflow-hidden" style={{ height: "80px" }}>
                          <div className="w-full bg-[#0F539C] dark:bg-blue-500 rounded-t-md transition-all duration-700" style={{ height: `${pct}%`, marginTop: `${100 - pct}%` }} />
                        </div>
