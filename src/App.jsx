...
function RoleBasedIndex() {
    ...
    const syncUser = async () => {
      try {
        ...
        if (userData && userData.role_id) {
          const fetchedRole = ROLE_MAP[userData.role_id] || 'Solicitante';
          setRole(fetchedRole);
          return;
        }
        
        // 2. Si no existe, vemos si la persona existe por correo
        ...
        // 3. Creamos al usuario con role_id = 1 (Solicitante por defecto)
        await supabase
          .from("identity_users")
          .insert({
             clerk_user_id: user.id,
             person_id: personId,
             role_id: 1, // 1 = Solicitante, 3 = Admin
             is_active: true
          });
        ...
        console.error("Error al sincronizar/obtener usuario:", err);
        setRole('Solicitante');
      }
    };
    syncUser();
  ...
  return <Dashboard /> // Administrador del Sistema
}

// Protects a route by role. Redirects to /app if the user's role is not in allowedRoles.
